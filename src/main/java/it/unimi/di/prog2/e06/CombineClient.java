...
public class CombineClient {
    ...
    /**
     * given a sequence of integer a and b from input stream, print
     * on console the the result of the multiplication of each element of a
     * for the sum of all elements of b
     * 
    ...
    public static void main(String[] args) throws IllegalArgumentException{
        ...
        try {
            ...
            //a = (Arrays.asList(Integer.getInteger(containerA.split(" "))));

            String[] temp = containerA.split(" ");
            for (String i : temp) {
                a.add(Integer.parseInt(i));
            }

            temp = containerB.split(" ");
            for (String i : temp) {
                b.add(Integer.parseInt(i));
            }

        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("CombineClient.main");
        }


        int multip = 0;
        try {
            multip = Combine.sumList(b);
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException("CombineClient.main");
        }

        List<Integer> result = Combine.multiplyList(a, multip);
